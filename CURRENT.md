...
- [x] Merged server logs and command input into a single Console panel. New containers are created with stdin open/attachable so server console commands can reach processes that read stdin.
...
- Docker stdin console support requires containers to be created with stdin open. Existing containers created before this change may need to be recreated before console input works.
